            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form
                        ...
                        class="space-y-8"
                        x-data="{ type: @js(old('formation_type', $formation->type)), parentId: @js(old('parent_id', $formation->parent_id)) }"
                    >
                        @csrf
                        @method('PATCH')
                        ...
                        <div>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <x-text-input id="formation_name" name="formation_name" type="text" class="mt-1 block w-full" :value="old('formation_name', $formation->name)" required autofocus />
                                    <x-input-error class="mt-2" :messages="$errors->get('formation_name')" />
                                </div>

                                <div>
                                    <x-input-label for="formation_code" :value="__('Code')" />
                                    <x-text-input id="formation_code" name="formation_code" type="text" class="mt-1 block w-full" :value="old('formation_code', $formation->code)" required />
                                ...
                                <div>
                                    ...
                                    <select id="formation_type" name="formation_type" x-model="type" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                                        ...
                                        @foreach($types as $t)
                                            <option value="{{ $t }}" {{ old('formation_type', $formation->type) === $t ? 'selected' : '' }}>{{ $t }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div x-show="type && type !== 'Zonal Command'" x-cloak>
                                    ...
                                    <select id="parent_id" name="parent_id" x-model="parentId" :required="type && type !== 'Zonal Command'" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                                        ...
                                        @foreach($zonalCommands as $zonal)
                                            <option value="{{ $zonal->id }}" {{ (string) old('parent_id', $formation->parent_id) === (string) $zonal->id ? 'selected' : '' }}>
                                                {{ $zonal->name }}
                                            </option>
                                        @endforeach
                                    </select>
